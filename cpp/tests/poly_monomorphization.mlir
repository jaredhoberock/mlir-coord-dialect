...
trait.impl @Coord for !coord.poly<0> {
  func.func private @add(%a: !coord.poly<0>, %b: !coord.poly<0>) -> !coord.poly<0> {
    %res = coord.add %a, %b : !coord.poly<0>
    return %res : !coord.poly<0>
  }

  func.func private @sub(%a: !coord.poly<0>, %b: !coord.poly<0>) -> !coord.poly<0> {
    %res = coord.sub %a, %b : !coord.poly<0>
    return %res : !coord.poly<0>
  }

  func.func private @inner_product(%a: !coord.poly<0>, %b: !coord.poly<0>) -> i64 {
    %res = coord.inner_product %a, %b : !coord.poly<0>
    return %res : i64
  }
}
